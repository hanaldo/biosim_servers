function Hive(game, actor, x, y) {
    ...
    if (GameType === "BeeSim") {
        hiveImage = "bee-hive";
        if (actor.name !== "g1") {
            hiveImage = "bee-other-hive";
        }
    } else if (GameType === "AntSim") {
        hiveImage = "ant-hive";
        if (actor.name !== "g1") {
            hiveImage = "ant-other-hive";
        }
    }
    ...
    hive.events.onDragStop.add(function () {
        ...
        var newX = Math.round(gameControl.getFakeX(hive.x));
        var newY = Math.round(gameControl.getFakeY(hive.y));
    ...
    this.reScale = function (size) {
        ...
        if (size >= 50) {
            settingIcon.scale.setTo(scale * 0.35);
    ...
    this.applySetting = function (current, max, dance, nec, doTrail, recruitAtHome) {
        var newActorSetting = {
            id: actor.id,
            dyProps: {
    ...
    function makeSettingIcon() {
        ...
        settingIcon.events.onInputDown.add(function () {
            BootstrapDialog.show({
                ...
                title: "<span class='glyphicon glyphicon-cog'></span>Setting for Colony " + actor.id,
                ...
                onshown: function () {
                    ...
                    angular.element(document.getElementById("root_control")).injector().invoke(function ($compile) {
                        ...
                        scope.doTrail = actor.dyProps.doTrail;
                        scope.recruitAtHome = actor.dyProps.recruitAtHome;
