function RootControl($scope, $upload, $http, $filter) {
    ...
    $scope.applyActorSetting = function () {
        ...
        if (currentSettingActor) {
            if (currentSettingActor.type === "f") {
                ...
                currentSettingActor.model.applySetting($scope.nLevel, $scope.nMax, $scope.dance, $scope.nectarQuality);
            }
        }
        BootstrapDialog.closeAll();
    };
    ...
    $scope.onFileSelect = function ($files) {
        ...
        reader.onload = function (e) {
            lastJsonFile = reader.result;
            $scope.canSaveReplay = false;
            gameControl.topText = "Replay: " + file.name;
    ...
    $scope.downloadReplay = function (index) {
        var replay = $scope.allReplays[index];
        if (index === 0) {
            window.open("getGameState", '', 'width=800,height=600');
        } else {
            window.open(replay.filePath, '', 'width=800,height=600');
        }
    };
    ...
    /*
     ...
     BeeSign control
     */
    $scope.copySpaceToRight = function () {
        ws.send("@copyToRight");
    };

    $scope.copySpaceToLeft = function () {
        ws.send("@copyToLeft");
    };

    $scope.enterBeeSignMode = function () {
        var r = confirm("Enter BeeSign mode will destroy current live game, proceed?");
        if (r == true) {
            $.ajax({
                url: "beesign",
                type: "post",
    ...
    $scope.startBeeSign = function () {
        ws.send("@goScout#1000#200#120");
    };

    $scope.pauseBeeSign = function () {
        ws.send("@pauseSign");
    };
